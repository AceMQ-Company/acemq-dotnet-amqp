...
#
# It runs first because it needs neither pandoc nor docfx, and a bad link is
...
for page in pages:
    ...
    for target in re.findall(r"\]\(([^)\s]+)\)", body):
        ...
        if not path:
            continue
        links += 1
        # The generated API reference is genuinely HTML: DocFX writes it from the
        # XML documentation comments, no markdown in docs/ renders into it, and it
        # is .html under apidocs/ on the site exactly as written. Any other .html
        # target is a docs page written the wrong way round -- it is the mistake
        # this check exists for, so it is named as that rather than reported as a
        # missing file, which is what it would look like from here.
...
FOOT='</main>
<footer>
  <a href="https://github.com/AceMQ-Company/acemq-dotnet-amqp">AceMQ for .NET</a> &mdash;
...
for f in docs/*.md; do
  ...
  pandoc "$f" \
    ...
    --metadata pagetitle="$title — AceMQ for .NET" \
    "${HIGHLIGHT[@]}" \
    --css=style.css \
