...
Resolution has two halves. One is the schema the writer used, which comes off the
registry. The other is the schema this consumer was written against — the **reader
schema** — and it is `codec.ReaderSchema`. Given both, Avro reconciles them: a field
the writer added is skipped, and a field the writer has not started sending yet
arrives as the reader schema's default. The handler sees the shape it was compiled
against, whichever version produced the message.

By default the reader schema is the schema the codec was built with, which is what
Python's `reader_schema` and Ruby's `reader_schema:` also default to. Two calls
change it.

**A different reader schema**, for a service that publishes one version and consumes
another. Only `schema` is registered; `readerSchema` is never written:

```csharp
var codec = AvroCodec.Registered(registry, publishedSchemaJson, consumedSchemaJson);
```

That is Java's `registered(registry, readerSchema)` under the same name.

**No reader schema at all**, so every message is read with the shape its writer gave
it:

```csharp
var codec = AvroCodec.Registered(registry, schemaJson).WithoutReaderSchema();
// codec.ReaderSchema is null
```

The opt-out, and what Java and Go do when a registered codec is given no reader
schema. It is for a consumer that wants to see exactly what a producer sent — a
bridge, an inspector, a dead-letter drainer reading versions it was never compiled
against. What it costs is what resolution buys: a field the writer added arrives
rather than being skipped, and a field the writer has not started sending is absent
rather than defaulted, so the target type has to match what the writer actually
wrote, version by version.

`WithoutReaderSchema()` returns a new codec and leaves the one it was called on
alone. On a fixed-schema codec it throws: nothing is framed, so there is no writer's
schema to read with and the codec's own is the only one there is.
...
`tests/AceMq.Amqp.Tests/fixtures/avro-resolution-fixtures.json`,
...
### Asking for resolution in .NET

```csharp
// Resolves: every message is reconciled with the schema this consumer holds.
// This is the ordinary Registered(...) call -- there is nothing to switch on.
var codec = AvroCodec.Registered(registry, schemaJson);

// Resolves onto a schema other than the one it writes. Only `schemaJson` is
// registered; `readerSchemaJson` is never written.
var consumer = AvroCodec.Registered(registry, schemaJson, readerSchemaJson);

// Does not resolve: the record arrives in the shape the producer wrote it.
var asWritten = AvroCodec.Registered(registry, schemaJson).WithoutReaderSchema();
```
...
read side. See [the reader schema](#the-reader-schema) above for what each of the
three does to `codec.ReaderSchema`.

One edge worth knowing before you meet it: `WithoutReaderSchema()` throws on a
fixed-schema codec, the one built with `AvroCodec.Of(...)`. Nothing is framed
there, so there is no writer's schema to read with and the codec's own is the only
one there is — turning resolution "off" would quietly change nothing. The other
edge is the decode target. Without a reader schema the shape changes from message
to message, so a plain class mapped onto one version reads the versions that match
it and nothing else; decode into `GenericRecord` and look at
`record.Schema.Fields` if what you are reading is a queue of mixed versions.
